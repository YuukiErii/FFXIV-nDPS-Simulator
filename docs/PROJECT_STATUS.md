...
| NIN | `examples/skill_lines/nin_m12s_p2/nin_830.csv` | `official_7_5_mechanics_checked`; `mechanic_calibrated` | Mudra/Ninjutsu, Kassatsu, Ten Chi Jin/Tenri Jindo, Shadow Walker/Hidden gates, Dokumori, Kunai's Bane, Meisui/Higi, Bunshin, Raiju, Kazematoi, Doton/Hollow Nozuchi, Dream Within a Dream shared crit/direct-hit rolls, default-hit positionals, and retained calibrated debuff windows are modeled. Seek real-log or independently audited numerical validation; the retained axis still has two intentional Ninki-overcap warnings. |
...
| DRG | `examples/skill_lines/drg_xivintheshell_long/drg_xivintheshell_long.csv` | `official_7_5_mechanics_checked`; `xivintheshell_damage_compared` | Power Surge and Life of the Dragon are consumed from the shared skill-buff path without double counting; Lance Charge, Battle Litany, Life Surge, Nastrond/Stardiver/Starcross readiness, jump follow-ups, Draconian Fire, Firstminds' Focus, Wyrmwind, Chaotic Spring DoT, and 7.5 Starcross potency are modeled. Seek real-log numerical validation next. |
